...
import xcj.app.main.model.redis.TokenInRedisWrapper
...
class TokenHelper(private val stringRedisTemplate: StringRedisTemplate) {
    ...
    fun generateTokenAndSaveToRedis(uid: String, loginParams: LoginParams, canMultiOnline: Boolean): String {
        val tokenExpireDuration = Duration.ofDays(30)
    ...
    private fun generateTokenAndSaveToRedis(
        ...
        tokenInRedisWrapper.loginParams
            ?: throw ApiDesignPermissionException("tokenInRedisWrapper.loginParams is null!")
        ...
        if (tokenInRedisWrapper.canMultiOnline == false && tokenFoundedInRedis != null) {
            PurpleLogger.current.d(
                TAG,
                "generateTokenAndSaveToRedis, uid:$uid Your current login info is not same as last login info, your last login will become invalid! because your account can't multi device online!"
            )
        ...
        valueOps.set(
            realTokenInRedis,
            tokenInRedisWrapper.jsonStr(gson),
            withExpiresAt.time - genDate.time,
            TimeUnit.MILLISECONDS
        )
        //多设备可同时登录[在线]
        if (tokenInRedisWrapper.canMultiOnline == true) {
            PurpleLogger.current.d(TAG, "generateTokenAndSaveToRedis, uid:${uid} Your account can multi device online!")
            if (count != null) {
                if (count == 0L) {
            ...
            if (count == 0L) {
                PurpleLogger.current.d(
                    TAG,
                    "generateTokenAndSaveToRedis, uid:${uid} Your account can only login on one device at the same time!"
                )
                zSetOps.add(uid, realTokenInRedis, 0.0)
            } else if (count == 1L) {
