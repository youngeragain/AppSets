...
sealed class NetState(isAvailable: Boolean){
    data class WIFI(val available:Boolean):NetState(available)
    data class FourGen(val available:Boolean):NetState(available)
    data class ForthGenPlus(val available:Boolean):NetState(available)
    data class FifthGen(val available:Boolean):NetState(available)
}
...
class NetStateBinder: Binder() {
    ...
    fun isListenersEmpty():Boolean = netStateListeners.size==0
