...
class StreamReadMethod(
    ...
    companion object {
        private const val TAG = "StreamReadMethod"
    }
    ...
    private val inputStream: BufferedInputStream =
        socket.inputStream.buffered(ReadThread.Companion.READ_BUFFER_SIZE)
    ...
    private val bufferSize = ReadThread.Companion.READ_BUFFER_SIZE
        ...
        while (!readThread.isExit()) {
            ...
            if (headerCountPosition < 4) {
                ...
                if (headerContentPosition + 1 < headerCount) {
                    headerContentBytes?.set(headerContentPosition++, byte.toByte())
                } else {
                        ...
                        if (file.canWrite()) {
                            dataContent =
                                DataContent.FileContent(
                                    file,
                                    file.outputStream().buffered(WRITE_BUFFER_SIZE)
                                )
                            PurpleLogger.current.d(
                                TAG,
     ...
     */
    private fun readContentChunk(byteArrayBuffer: ByteArray, startIndex: Int, length: Int) {
            ...
            ContentType.APPLICATION_FILE -> {
                ...
                try {
                    ...
                    if (content is DataContent.ByteArrayContent) {
                        ...
                        System.arraycopy(byteArrayBuffer, 0, newBytes, lastBytes.size, length)
