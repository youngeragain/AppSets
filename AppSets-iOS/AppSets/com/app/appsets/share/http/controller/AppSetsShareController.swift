...
struct AppSetsShareController<ReqContext: RequestContext> {
    ...
    @Sendable func greeting(_ request: Request, reqContext: ReqContext) async -> DesignResponse<String> {
    ...
    @Sendable func ping(_ request: Request, reqContext: ReqContext) async -> DesignResponse<String> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        return await service.ping(context: context, clientHost: clientHost)
    }

    @Sendable func isNeedPin(_ request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        return await service.isNeedPin(context: context, clientHost: clientHost)
    }

    @Sendable func pair(_ request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let pin = 0
    ...
    @Sendable func pairResponse(_ request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
    ...
    @Sendable func prepareSend(_ request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
        let uri = ""
    ...
    @Sendable func prepareSendResponse(_ request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
        let isAccept = false
    ...
    @Sendable func postText(request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
        let text = ""
    ...
    @Sendable func postFile(request: Request, reqContext: ReqContext) async -> DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
    ...
    @Sendable func postFileChunked(request: Request, reqContext: ReqContext) async ->DesignResponse<Bool> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
        let fileId = ""
    ...
    @Sendable func getContent(request: Request, reqContext: ReqContext) async -> DesignResponse<ContentDownloadN> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
        let contentId = ""
    ...
    @Sendable func getContentList(_ request: Request, reqContext: ReqContext) async -> DesignResponse<ContentInfoList> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
        let shareToken = ""
    ...
    @Sendable func exchangeDeviceInfo(request: Request, reqContext: ReqContext) async -> DesignResponse<HttpShareDevice> {
        let context = findContext()
        let clientHost = findClientHost(request, reqContext: reqContext)
