...
class SimpleUserRoleServiceImpl(
    ...
    override fun addRoleByUid(uid: String, addRoleParams: AddRoleParams): DesignResponse<Boolean> {
        ...
        if(userHasRole)
            return DesignResponse(ApiDesignCode.ERROR_CODE_FATAL, "The userId:${addRoleParams.uid} has  role:${addRoleParams.role}!", false)
        val addRoleResult =
            roleDao.addRole(addRoleParams.uid, addRoleParams.role, uid, addRoleParams.addReason)
        return if(addRoleResult==1)
            DesignResponse(info = "Add role:${addRoleParams.role} for userId:${addRoleParams.uid} successful!", data = true)
        else
            DesignResponse(ApiDesignCode.ERROR_CODE_FATAL, "Add role:${addRoleParams.role} for userId:${addRoleParams.uid} failed!", data = false)
    }
    ...
    override fun deleteRoleByUid(uid: String, deleteRoleParams: DeleteRoleParams): DesignResponse<Boolean> {
        ...
        if(!userHasRole)
            return DesignResponse(ApiDesignCode.ERROR_CODE_FATAL, "The userId:${deleteRoleParams.uid} does not have role:${deleteRoleParams.role}!", false)
        ...
        return if(deleteUserRoleResult==1)
            DesignResponse(info = "Delete userId:${deleteRoleParams.uid}} role:${deleteRoleParams.role} successful!", data = true)
        else
            DesignResponse(info = "Delete userId:${deleteRoleParams.uid}} role:${deleteRoleParams.role} failed!", data = false)
    }
    ...
    override fun updateRoleByUid(uid: String, updateRoleParams: UpdateRoleParams): DesignResponse<Boolean> {
        ...
        if(!userHasRole)
            return DesignResponse(ApiDesignCode.ERROR_CODE_FATAL, "The userId:${updateRoleParams.uid} has not role:${updateRoleParams.oldRole}", false)
        ...
        return if(updateUserRoleResult==1)
            DesignResponse(info = "Update userId:${updateRoleParams.uid} old role:${updateRoleParams.oldRole} to new role:${updateRoleParams.newRole} successful!", data = true)
        else
            DesignResponse(info = "Update userId:${updateRoleParams.uid} old role:${updateRoleParams.oldRole} to new role:${updateRoleParams.newRole} failed!", data = false)
    }
