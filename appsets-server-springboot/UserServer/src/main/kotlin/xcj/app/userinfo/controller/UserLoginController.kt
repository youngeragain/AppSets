...
import xcj.app.*
import xcj.app.userinfo.ktx.produceExceptionIfBindingResultContains
...
import xcj.app.userinfo.service.UserLoginService
...
class UserLoginController(
    ...
    fun logIn(@Validated @RequestBody loginParams: LoginParams, bindingResult:BindingResult):DesignResponse<String>{
    ...
    fun logInByOtherDevice(@RequestHeader(name = ApiDesignEncodeStr.tokenStrToMd5) token:String, @Validated @RequestBody loginParams: LoginParams):DesignResponse<String>{
        return loginService.logInByOtherDevice(token, loginParams)
    }

    @ApiDesignPermission.VersionRequired(200)
    @RequestMapping("signup", method = [RequestMethod.POST])
    fun signUp(@Validated @RequestBody signupParams: SignupParams):DesignResponse<Boolean>{
        return loginService.signUp(signupParams)
    }

    @ApiDesignPermission.VersionRequired(200)
    @RequestMapping("signup/pre", method = [RequestMethod.POST])
    fun preSignUp(@RequestParam("ac", required = true) account:String):DesignResponse<Boolean>{
    ...
    fun signOut(@RequestHeader(name = ApiDesignEncodeStr.tokenStrToMd5) token:String):DesignResponse<Boolean>{
    ...
    fun logOut(@RequestHeader(name = ApiDesignEncodeStr.tokenStrToMd5) token:String,
               @PathVariable(name = "deleteData") deleteData:Boolean):DesignResponse<String>{
    ...
    fun logOutAdmin(@PathVariable(name = "deleteData") deleteData:Boolean,
                       @PathVariable(name = "userId") userId: String):DesignResponse<String>{
        return loginService.logOutByUserId(userId, deleteData)
    }

    @RequestMapping("mongo/add", method = [RequestMethod.POST])
    fun addUserToMongo(@RequestBody user: User):DesignResponse<String>{
