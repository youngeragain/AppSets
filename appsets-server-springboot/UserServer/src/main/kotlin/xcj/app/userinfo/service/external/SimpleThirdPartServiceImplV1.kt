...
import xcj.app.userinfo.service.external.ThirdPartKeys.TENCENT_COS
import java.util.*

object ThirdPartKeys{
    const val TENCENT_COS = "thirdpart_tencent_cos"
}
...
class SimpleThirdPartServiceImplV1(val redisTemplate: StringRedisTemplate):ThirdPartService{
    ...
    override fun getTencentCosSTS(): DesignResponse<TencentCosSTS?> {
        try {
            ...
            val subAccountSecretId = opsForHash.get(TENCENT_COS, "SecretId")
            val subAccountSecretKey = opsForHash.get(TENCENT_COS, "SecretKey")
            val subAccountBucketName = opsForHash.get(TENCENT_COS, "BucketName")
            val subAccountRegion = opsForHash.get(TENCENT_COS, "Region")
            val subAccountSTSHost = opsForHash.get(TENCENT_COS, "STSHost")//"sts.internal.tencentcloudapi.com"
            val subAccountSTSDuration = opsForHash.get(TENCENT_COS, "STSDuration")
            val subAccountSTSAllowPrefixes = opsForHash.get(TENCENT_COS, "STSAllowPrefixes")
            val subAccountSTSAllowActions = opsForHash.get(TENCENT_COS, "STSAllowActions")
    ...
    override fun getTencentCosRegionBucket(): DesignResponse<TencentCosRegionBucket?> {
        ...
        val subAccountBucketName = opsForHash.get(TENCENT_COS, "BucketName")
        val subAccountRegion = opsForHash.get(TENCENT_COS, "Region")
        val subAccountFilePathPrefix = opsForHash.get(TENCENT_COS, "FilePathPrefix")
        if(subAccountBucketName.isNullOrEmpty()||subAccountRegion.isNullOrEmpty()||subAccountFilePathPrefix.isNullOrEmpty())
            return DesignResponse()
        val tencentCosRegionBucket = TencentCosRegionBucket(subAccountRegion, subAccountBucketName, subAccountFilePathPrefix).apply {
            encode()
        }
